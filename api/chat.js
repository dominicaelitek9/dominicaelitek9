...
// Initialize the SDK using Vercel's secure server environment variables
const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });

export default async function handler(req, res) {
    // Enable CORS and options handling so your frontend can call this function safely
    ...
    try {
        ...
        if (!message) {
            return res.status(400).json({ error: 'Missing user message string in payload.' });
        }

        // MASTER AI SYSTEM RULES PROFILE
        const systemInstruction = `
        You are the highly comprehensive, intelligent, and articulate Command AI Matrix receptionist built for the website of Dominica Elite K9 Services located in Portsmouth, Dominica. 
        ...
        // Request generation using the standard, fast text model
        ...
        const aiReplyText = response.text || "Signal processed. Please contact our Portsmouth command center on WhatsApp for details.";
        
        return res.status(200).json({ reply: aiReplyText });

    } catch (error) {
        console.error("Internal Serverless Matrix Error:", error);
